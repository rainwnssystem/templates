...
def compression(args):
    with zipfile.ZipFile(f'{args.dataset}/dataset.zip', 'r') as zip_ref:
...
def train(args):
    train_dataset = keras.utils.image_dataset_from_directory(
        f'{args.dataset}/dataset/train',
        image_size=(32, 32),
        batch_size=args.batch_size,
        shuffle=True,
    )
    test_dataset = keras.utils.image_dataset_from_directory(
        f'{args.dataset}/dataset/test',
        image_size=(32, 32),
        batch_size=args.batch_size,
        shuffle=False,
    )
    ...
    best_path = os.path.join(model_dir, "best.weights.h5")
...
def save_model(model, model_dir):
    ...
    path = os.path.join(model_dir, "1")

    @tf.function(input_signature=[tf.TensorSpec(shape=[None, 32, 32, 3], dtype=tf.float32)])
    def serving_default(x):
        return model(x, training=False)

    tf.saved_model.save(model, path, signatures={"serving_default": serving_default})


# required for inference (TF Serving handlers)
